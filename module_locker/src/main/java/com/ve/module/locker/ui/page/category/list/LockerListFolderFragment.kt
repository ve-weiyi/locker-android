...
import com.ve.module.locker.ui.page.category.details.LockerDetailsFolderFragment
...
class LockerListFolderFragment :
    BaseVmListFragment<LockerLayoutListCategoryBinding, LockerPrivacyCategoryViewModel, PrivacyFolder>() {
    ...
    private fun itemChildClick(item: PrivacyFolder, view: View, position: Int) {
        when (view.id) {
            R.id.item_layout_content -> {
                val bundle = Bundle()
                bundle.putInt(LockerDetailsFolderFragment.FRAGMENT_TYPE_KEY,EditType.SEE_TAG_TYPE)
                bundle.putSerializable(LockerDetailsFolderFragment.FRAGMENT_DATA_KEY, item)
                LockerContainerActivity.start(mContext, LockerDetailsFolderFragment::class.java.name, "查看标签 "+item.folderName,bundle )

                showMsg("查看")
            }
            R.id.item_btn_edit -> {
                val bundle = Bundle()
                bundle.putInt(LockerDetailsFolderFragment.FRAGMENT_TYPE_KEY,EditType.EDIT_TAG_TYPE)
                bundle.putSerializable(LockerDetailsFolderFragment.FRAGMENT_DATA_KEY, item)
                LockerContainerActivity.start(mContext, LockerDetailsFolderFragment::class.java.name, "编辑标签 "+item.folderName, bundle)
                showMsg("编辑")
            }
            R.id.item_btn_delete -> {
                mViewModel.folderDelete(item.id!!)
                showMsg("删除")
            }
        }
    }
    ...
    private fun addPrivacyFolder() {
        val bundle = Bundle()
        bundle.putInt(LockerDetailsFolderFragment.FRAGMENT_TYPE_KEY,EditType.ADD_TAG_TYPE)
        LockerContainerActivity.start(mContext, LockerDetailsFolderFragment::class.java.name, "添加标签",bundle)
        showMsg("添加")
    }
