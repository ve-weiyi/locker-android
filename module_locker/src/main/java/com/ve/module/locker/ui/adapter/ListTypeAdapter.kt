 ...
 * @Date 2022/4/9
 */
class ListTypeAdapter: BaseSlideBindingAdapter<PrivacyType, LockerItemSimpleListBinding>() {
    ...
    override fun convert(holder: VBViewHolder<LockerItemSimpleListBinding>, item: PrivacyType) {
        holder.vb.tvTilt.setText(item.id.toString())
        holder.vb.itemId.text = item.id.toString()
        holder.vb.itemName.text = item.typeName
        holder.vb.itemDesc.text = item.typeDesc
    }
}